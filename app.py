...
"""
            ...
            try:
                ...
                # ------------------------------
                # MARKING RUBRIC TABLE (OPTION B)
                # ------------------------------

                st.subheader("Marking Rubric Overview")
                st.write("This table summarises how each assessment criterion was evaluated:")
                ...
                # Pattern to catch criteria like LO1 – P1, LO2-P4, LO3 – M3 etc.
                criterion_pattern = r"(LO\d+\s*[-–]\s*[PMD]\d+)"
                ...
                for line in lines:
                    ...
                    if any(key in lower for key in statuses):
                        criterion_match = re.search(criterion_pattern, line, re.IGNORECASE)

                        criterion_label = criterion_match.group(1) if criterion_match else "Unknown criterion"

                        for key, status_text in statuses.items():
                            if key in lower:
                                rubric_rows.append([status_text, criterion_label])
                                break

                if rubric_rows:
                    df = pd.DataFrame(rubric_rows, columns=["Status", "Learning Outcome / Criterion"])
                    st.table(df)
                else:
                    st.info("No criteria could be extracted for rubric display.")
